...
export default function CatCollection() {
  const [filterCoat, setFilterCoat] = useState<CatCoat | 'all'>('all');
  ...
  const filteredCats = useMemo(() => {
    ...
    if (filterCoat !== 'all') cats = cats.filter(c => c.coat === filterCoat);
    ...
    return cats;
  }, [filterCoat, filterListed, sortBy]);
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="sticky top-0 z-20 border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3">
          <select value={filterCoat} onChange={(e) => setFilterCoat(e.target.value as CatCoat | 'all')} className="rounded-lg border border-white/10 bg-gray-900 px-3 py-1.5 text-sm text-white">
            <option value="all">All Coats</option>
            {Object.keys(CAT_COAT_COLORS).map(coat => <option key={coat} value={coat}>{coat}</option>)}
          </select>
...
function CatCard({ cat, onClick }: { cat: CatEVO; onClick: () => void }) {
  ...
  const coatColor = CAT_COAT_COLORS[cat.coat];
  const scale = 0.5 + Math.min(1, cat.lockedSol / 50) * 0.5;

  return (
    <div onClick={onClick} className="group relative cursor-pointer overflow-hidden rounded-xl border border-white/10 bg-gradient-to-b from-gray-900 to-gray-950 transition-all hover:border-white/30 hover:scale-[1.02]" style={{ boxShadow: cat.isListed ? `0 0 20px ${coatColor}40` : 'none' }}>
      ...
      <div className="relative flex h-48 items-center justify-center overflow-hidden">
        <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 50% 50%, ${coatColor}15, transparent 60%)` }} />
        ...
        <img src="/cats/cat.png" alt={cat.name} className="relative z-[1] pixelated" style={{ transform: `scale(${scale})`, imageRendering: 'pixelated', filter: `drop-shadow(0 0 8px ${coatColor}) hue-rotate(${cat.hueShift}deg) saturate(${0.8 + (cat.facetCount / 100) * 0.8})` }} />
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-800">
          <div className="h-full transition-all" style={{ width: `${cat.facetCount}%`, background: `linear-gradient(90deg, ${coatColor}, #fff)` }} />
        </div>
      </div>
      <div className="p-3">
        ...
        <div className="mt-1 flex items-center gap-2 text-xs text-gray-400">
          <span style={{ color: coatColor }}>● {cat.coat}</span><span>·</span><span>{CAT_STAGE_NAMES[stage]}</span>
        </div>
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-white/10 bg-gradient-to-b from-gray-900 to-gray-950 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-6">
          <div className="relative flex h-64 w-64 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black/50">
            <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 50% 50%, ${coatColor}20, transparent 70%)` }} />
            <img src="/cats/cat.png" alt={cat.name} className="relative z-[1] pixelated" style={{ transform: `scale(${scale})`, imageRendering: 'pixelated', filter: `drop-shadow(0 0 12px ${coatColor}) hue-rotate(${cat.hueShift}deg) saturate(${0.8 + (cat.facetCount / 100) * 0.8})` }} />
            {cat.fractureLines.length > 0 && (
          ...
          <div className="flex-1">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="rounded-lg border border-white/10 bg-white/5 p-3"><span className="text-gray-500">Stage</span><p className="text-lg font-bold" style={{ color: coatColor }}>{CAT_STAGE_NAMES[stage]}</p></div>
              ...
              <div className="rounded-lg border border-white/10 bg-white/5 p-3"><span className="text-gray-500">Coat</span><p className="text-lg font-bold" style={{ color: coatColor }}>{cat.coat}</p></div>

...
Every EVO goes through this lifecycle. Some crystals are forged, fed, and held forever. Others are traded dozens of times. Each path creates a unique object with unique art.
...
Result: Empty crystal PDA created
...
**Key points:**
- Mint price is **fixed** — same for all 2,000 crystals
- The crystal starts **empty** — no SOL locked inside yet
- The resonance seed is **deterministic** from the forge transaction signature — unpredictable but permanent
- The resonance seed determines the crystal's base color palette and shape tendency
...
**What happens:** The owner deposits SOL into their crystal. This is the core "store of value" action.

```
User feeds: 1 SOL
  → 1 SOL added to crystal's locked_lamports
  → Crystal grows visually (more SOL = bigger crystal)
  → Fee: 0 SOL (or 0.001 SOL to treasury)

Result: locked_lamports increases
  → Art updates: crystal is now bigger
```
...
- **Feeding is what makes the crystal grow** — size = total SOL locked
- **Fee is zero or near-zero** — we want feeding to be frictionless

**Why feeding instead of fixed-price minting:**
- Fixed-price minting means only rich people get big crystals
- Feeding means anyone can start small and grow over time
- A crystal that was fed 0.1 SOL every week for a year tells a different story than one that was fed 50 SOL in one shot — even if the total is the same
- This creates **diverse crystal histories** — some are slowly grown, some are instantly massive

---

## 3. Growing (Time Evolution)

**What happens:** The crystal gains facets over time — automatically, no action required.
...
Example:
  Crystal forged 30 days ago → 4 facets
  Crystal forged 365 days ago → 52 facets
  Crystal forged 1 day ago → 0 facets
```

**Key points:**
- **Automatic** — no transaction needed. Facets are computed from `forged_at` timestamp
- **Continuous** — the crystal is always growing, even while you sleep
- **More facets = more intricate art** — the generative algorithm adds complexity per facet
- **Time held is total** — trading doesn't reset the timer. A crystal forged 2 years ago that was traded 5 times still has 2 years of facets
...
**What happens:** Owner lists crystal for sale, buyer pays, ownership transfers.

```
Step 1: Owner lists crystal
  → Sets asking price (e.g., 8 SOL)
  → Crystal marked as "listed" in program state
...
Step 3: Art updates
  → New fracture line appears on the crystal
  → Buyer sees their new crystal with its full history
```
...
FractureLine {
  ...
  timestamp: i64,          // when
  position: u8,            // where on the crystal (deterministic from trade data)
  intensity: u8,            // how visible (based on trade price relative to locked SOL)
}
```

A crystal traded 20 times has 20 fracture lines — a rich, cracked, storied appearance. A crystal never traded has zero fracture lines — clean, pristine. Both are beautiful in different ways.

---

## 5. Shattering (Redemption)

**What happens:** Owner destroys the crystal and reclaims all locked SOL.
...
User calls shatter()
  ...
  → Art is gone forever
  → Crystal ceases to exist
```

**Key points:**
- **Always available** — no lockup, no waiting period
- **Reclaims 99% of locked SOL** — 1% shatter fee to treasury
- **Destroys the crystal** — the art, the history, the facets — all gone
- **This is the price floor mechanism** — if market price drops below locked SOL, someone arbitrages by shattering
- **Creates scarcity** — shattering reduces total supply. Over time, crystals are destroyed, making remaining ones more rare
- **Irreversible** — once shattered, the crystal's resonance seed is gone forever. Even if you forge a new crystal, it gets a new seed.

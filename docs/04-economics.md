...
1. **Per-crystal economics** — how individual crystals are priced
2. **Collection economics** — how the Crystals collection works as a whole
...
Mint fee:
  ...
  → 0.04 SOL: Collection treasury (Crystals team)
...
A crystal with 5 SOL locked inside has a floor of 5 SOL. Why? Because you can always shatter it to reclaim 5 SOL (minus 1% fee = 4.95 SOL). No rational seller would accept less than 4.95 SOL — they'd shatter instead.
...
|---|---|
| Many facets (old crystal) | Time can't be faked. Old crystals are scarce. |
| Rare color palette | Some resonance seeds produce rare palettes |
| Rich fracture history | Traded many times = storied provenance |
| Pristine (never traded) | Clean crystals become rare as trading happens |
| Large size (lots of SOL) | Visible wealth = status symbol |
| Legendary status | Oldest + largest + most-traded crystals |

### Example Pricing Scenarios

```
Crystal A: 0.5 SOL locked, 1 week old, never traded
  → Floor: 0.5 SOL
  → Premium: ~0 (too new, too small)
  → Market price: ~0.5 SOL

Crystal B: 5 SOL locked, 6 months old, traded twice
  → Floor: 5 SOL
  → Premium: 2-5 SOL (decent age, some history)
  → Market price: 7-10 SOL

Crystal C: 50 SOL locked, 2 years old, traded 8 times, rare palette
  → Floor: 50 SOL
...
### Total Supply
```
Max crystals: 2,000 (hardcoded, can never increase)
```

### Supply Decreases Over Time
```
Crystals shattered → supply decreases → remaining crystals more scarce

Year 1:  ~2,000 crystals (some shattered)
Year 2:  ~1,600 crystals (more shattered)
Year 3:  ~1,200 crystals (survivors becoming rare)
Year 5:  ~800 crystals (legendary survivors)
```
...
Once all 2,000 crystals are minted, no new ones can ever be created. The only way to get one is to buy from an existing holder. Combined with decreasing supply from shattering, this creates strong scarcity pressure.

---

## Treasury

### Collection Treasury (Crystals)
Funds:
...
Example (hypothetical, mature ecosystem):
  Crystals:  500 trades/month × avg 10 SOL × 1% = 50 SOL/month
...
As more collections launch on EVO, protocol revenue grows — even if Crystals trading volume stays flat.
...
| **Collection** | Collect rare palettes, pristine crystals, legendary pieces. |
| **Status** | Owning a massive ancient crystal is a flex. Display it. |
...
| **Art appreciation** | The crystals are genuinely beautiful. Some people just want to own pretty things. |

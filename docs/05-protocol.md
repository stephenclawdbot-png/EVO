...
├── Crystals (our flagship collection, 2000 cap)
...
```
You deploy EVO program for Crystals only
Competitor copies code, deploys their own program
→ You earn: only from Crystal trades
...
```
Buyer pays X SOL for a crystal from collection Y:
...
- Token holders vote (governance token? or crystal-weighted voting?)
...
4. Drives attention back to Crystals (the original)
